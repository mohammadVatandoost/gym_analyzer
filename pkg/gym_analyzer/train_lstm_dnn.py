...
def train_lstm_dnn(
    ...
    train_data, train_labels = preprocess_on_key_points(
        pose, x_train, label_processor, data_path, Sequence_Length
    )
    test_data, test_labels = preprocess_on_key_points(
        pose, x_test, label_processor, data_path, Sequence_Length
    )
    logging.info(
        f"Training data shape: {train_data.shape} and size: {len(train_data)}, Training labels data shape: {train_labels.shape} and size: {len(train_labels)}"
    )
    logging.info(
        f"Testing data shape: {test_data.shape} and size: {len(test_data)}, Testing labels data shape: {test_labels.shape} and size: {len(test_labels)}"
    )
    # Callbacks to be used during neural network training
    # Stop training when a monitored metric has stopped improving.
    es_callback = EarlyStopping(
        monitor="val_loss", min_delta=5e-4, patience=10, verbose=0, mode="min"
    )
...
def build_lstm(label_processor):
    ...
    lstm.add(
        LSTM(
            ...
            activation="relu",
            input_shape=(Sequence_Length, Num_Input_Values),
        )
    )
